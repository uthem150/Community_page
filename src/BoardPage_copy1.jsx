...
const BoardPage = () => {
  ...
  customBars.forEach((customBar) => {
    ...
    if (originalText.length > maxCharacters) {
      ...
      textBody.textContent = truncatedText;

      const moreButton = document.createElement('button');
      moreButton.classList.add('more-text-button');
      moreButton.textContent = '더보기';

      customBar.appendChild(moreButton);

      moreButton.addEventListener('click', function () {
        textBody.textContent = originalText;
        this.remove();
        customBar.style.height =
          getComputedStyle(textBody).height + 'px';
      });
    }

    customBar.style.paddingBottom = '6rem';
  });
